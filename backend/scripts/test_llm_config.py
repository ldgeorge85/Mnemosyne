...
def test_persona_configs():
    ...
    print(f"Model Profile: {settings.LLM_MODEL_PROFILE}\n")
    
    personas = ["confidant", "mentor", "mediator", "guardian", "mirror"]
    
    for persona in personas:
        config = get_llm_config_for_persona(persona, settings.LLM_MODEL_PROFILE)
...
def test_model_profiles():
    ...
    print("\n=== Testing Model Profile Configurations ===")
    
    # Test with reasoning_channel profile
    os.environ["LLM_SUPPORTS_REASONING_LEVEL"] = "true"
    os.environ["LLM_MODEL_PROFILE"] = "reasoning_channel"
    ...
    print(f"Profile: {reloaded_settings.LLM_MODEL_PROFILE}")
    print(f"Supports Reasoning: {reloaded_settings.LLM_SUPPORTS_REASONING_LEVEL}\n")
    
    for persona in ["confidant", "guardian"]:
        config = get_llm_config_for_persona(persona, "reasoning_channel", reloaded_settings)
        print(f"{persona.capitalize()}:")
        print(f"  Temperature: {config.get('temperature')}")
        if config.get('system_prompt_prefix'):
...
def test_embedded_profile():
    """Test configuration for embedded system prompt profile."""
    print("\n=== Testing Embedded System Profile ===")
    
    os.environ["LLM_MODEL_PROFILE"] = "embedded_system"
    os.environ["LLM_SYSTEM_PROMPT_MODE"] = "embedded"
    ...
    config = get_llm_config_for_persona("guardian", model_profile="embedded_system", settings=embedded_settings)
    
    print(f"Profile: {embedded_settings.LLM_MODEL_PROFILE}")
    print(f"System Prompt Mode: {config.get('system_prompt_mode')}")
    print(f"Has Reasoning Prefix: {bool(config.get('system_prompt_prefix'))}")
    
    assert config.get('system_prompt_mode') == 'embedded', "Embedded profile should use embedded mode"
    assert not config.get('system_prompt_prefix'), "Embedded profile shouldn't have reasoning prefix"
    print("✅ Embedded system profile verified")
...
if __name__ == "__main__":
    ...
    os.environ.setdefault("LLM_MODEL_PROFILE", "standard")
